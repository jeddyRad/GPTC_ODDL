...
export function TaskModal({ task, onClose }: TaskModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div
        className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md max-w-4xl w-full max-h-[90vh] overflow-y-auto relative p-0"
      ...
      >
        ...
        {/* Sélecteur du type de tâche (UX moderne) */}
        {!task && (
          <div className="px-6 pt-4 pb-2 flex gap-4 items-center">
            <label className="font-medium text-gray-700 dark:text-gray-200">{t('task.type')}</label>
            <select
              className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 focus:ring-primary-500 focus:border-primary-500 text-sm shadow-none"
              value={taskType}
        ...
        {/* Formulaire dynamique selon le type de tâche */}
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          {/* SUPPRESSION DU SWITCHBOX TÂCHE PERSONNELLE */}
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Service (masqué si tâche personnelle) */}
            {taskType === 'service' && (
                  <div>
            ...
            {taskType === 'projet' && (
              ...
              {canEdit && (
                <div className="flex items-center justify-end gap-2 pt-6 border-t border-gray-200 dark:border-gray-700">
                  <Button
                    onClick={onClose}
                    variant="secondary"
                    leftIcon={<X />}
